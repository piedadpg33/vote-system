...
export default function VotingInProgressPage({ disconnect }) {
  ...
  async function handleCerrar(vote) {
    setBoton({ [vote.id]: 'Guardando resultados en la blockchain...' });
    setAccion({ [vote.id]: 'Por favor, confirma la transacción en tu wallet para guardar el resultado en la blockchain. Si usas WalletConnect, revisa tu app móvil.' });
    try {
      ...
      if (!res.ok) {
        setBoton({ [vote.id]: 'Cerrar votación' });
  ...
  return (
    <div style={{ marginTop: 60, textAlign: 'center' }}>
      ...
      <button onClick={() => handleCerrar(vote)}>
        {boton[vote?.id] || 'Cerrar votación'}
      </button>

      <div style={{ color: '#444', minHeight: 24, marginTop: 8 }}>
        {accion[vote?.id] || ''}
      </div>
